	...
	public static void GeraGeracoes() {
		...
		if (primeira) {
			...
			for (int i = 0; i < geracao.length; i++) {
				...
				somaM = obj.getQuantidadeBotasFemininas() + obj.getQuantidadeSandalia()
						+ obj.getQuantidadeSapatoMasculino() + obj.getQuantidadeSapatosFemininos();

				if (obj.getQuantidadeBotasFemininas() != 0) {

					auxilio = obj.BotasFemininas();

					somaB += (auxilio[3] * obj.getQuantidadeBotasFemininas());
					somaC += (auxilio[1] * obj.getQuantidadeBotasFemininas());
					somaT += (auxilio[2] * obj.getQuantidadeBotasFemininas());
				}

				if (obj.getQuantidadeSandalia() != 0) {

					auxilio = obj.Sandalia();

					somaB += (auxilio[3] * obj.getQuantidadeSandalia());
					somaC += (auxilio[1] * obj.getQuantidadeSandalia());
					somaT += (auxilio[2] * obj.getQuantidadeSandalia());
				}

				if (obj.getQuantidadeSapatoMasculino() != 0) {

					auxilio = obj.SapatoMasculino();

					somaB += (auxilio[3] * obj.getQuantidadeSapatoMasculino());
					somaC += (auxilio[1] * obj.getQuantidadeSapatoMasculino());
					somaT += (auxilio[2] * obj.getQuantidadeSapatoMasculino());
				}

				if (obj.getQuantidadeSapatosFemininos() != 0) {

					auxilio = obj.SapatosFemininos();

					somaB += (auxilio[3] * obj.getQuantidadeSapatosFemininos());
					somaC += (auxilio[1] * obj.getQuantidadeSapatosFemininos());
					somaT += (auxilio[2] * obj.getQuantidadeSapatosFemininos());
				}
			...
			for (int i = 0; i < objt.size(); i++) {
				...
				if (melhorGeracacao <= objt.get(i).getBeneficioTotal()
						&& objt.get(i).getQuantidadeMaterial() <= materialMaximo
						&& objt.get(i).getTempoDeCriacao() <= tempoMaximo
						&& objt.get(i).getCustoDeCriacao() <= custoMaximo) {
			...
			if (melhor) {
				...
				for (int i = 0; i < novaGeracao.length; i++) {
					for (int j = 0; j < novaGeracao.length; j++) {
						...
						if (novaGeracao[i][j] == 1 && somaM <= 400) {
